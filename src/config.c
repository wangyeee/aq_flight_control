...
float p[CONFIG_NUM_PARAMS] CCM_RAM = {0};
configData_t configData CCM_RAM;
...
void configGetBinaryParam(configBinary_t* cfg, uint16_t n) {
    cfg->magic = CONFIG_BIN_MAGIC;
    cfg->index = n;
    cfg->value = configGetParamValueForSave(n);
}

int configParseBinaryParams(uint8_t *fileBuf, int size) {
    int paramCnt;
    configBinary_t cfg;

    for (paramCnt = 0; size > 0; fileBuf += CONFIG_BINARY_PARAM_SIZE) {
        memcpy(&cfg, fileBuf, CONFIG_BINARY_PARAM_SIZE);
        size -= CONFIG_BINARY_PARAM_SIZE;
        if (cfg.magic == CONFIG_BIN_MAGIC) {
            configSetParamByID(cfg.index, cfg.value);
            paramCnt++;
        }
    }
    return paramCnt;
